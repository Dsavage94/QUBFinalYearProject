...
{
    public partial class TimetableLab2 : Form
    {
        Form previousForm;
        bool BackButtonPressed = false;
        public TimetableLab2(Form sender)
        {
            InitializeComponent();
            previousForm = sender;
        }
        ...
        private void TimetableLab2_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void TimetableLab2_Load(object sender, EventArgs e)
        {
        ...
        private void tblbookingsBindingNavigatorSaveItem_Click(object sender, EventArgs e)
        {
            this.Validate();
            this.tblbookingsBindingSource.EndEdit();
            this.tableAdapterManager.UpdateAll(this.dsavage11DataSet);
        ...
        private void colourTheButtons(Button currentButton)
        {
            ...
            {
                if (row["idLab"].ToString() == "LAB002")
                {
                    strLab2Computers[Lab2ComputerIndex] = row["idComputer"].ToString();
                    Lab2ComputerIndex++;
                }
            ...
            Lab2ComputerIndex = 0;
                ...
                {
                    if (Convert.ToInt32(row["idTimeSlot"]) == currentTimeSlot && row["idComputer"].ToString() == strLab2Computers[Lab2ComputerIndex])
                    {
                        Lab2Status[Lab2ComputerIndex, 0] = row["idBookings"].ToString();
                        Lab2Status[Lab2ComputerIndex, 1] = row["idComputer"].ToString();
                        Lab2Status[Lab2ComputerIndex, 2] = row["idTimeslot"].ToString();
                        Lab2Status[Lab2ComputerIndex, 3] = row["Status"].ToString();
                        Lab2ComputerIndex++;
                    }

                }
                for (int Lab2StatusIndex = 0; Lab2StatusIndex < Lab2Status.GetLength(0); Lab2StatusIndex++)
                {
                    if (Lab2Status[Lab2StatusIndex, 3].ToString() != "Open")
                    {
                        numberOfBookedComputers++;
                    }
                }
                currentLabPercentage = Convert.ToInt32((numberOfBookedComputers / 2));
        ...
        private void btnTimeSlotPicked_Click(object sender, EventArgs e)
        {
            ...
            MapLab2 M2 = new MapLab2(currentTimeSlot, ActiveForm);
            M2.Show();
            this.Hide();
        }
    }
